...
> 2) Dejar limpio el índice para poder hacer pull
: 
    * **opción a**: ```git commit```
    * **opción b**: ```git stash```

> 3) Obtener los cambios realizados en remoto mediante ```git pull```
: 
...
> 4) Volver a comprobar si hay nuevos cambios en remoto (por si alguien ha hecho push mientras nos encontrabamos en el punto anterior)
: ```git pull```
