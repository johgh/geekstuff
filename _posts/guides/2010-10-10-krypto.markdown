...
Los 4 problemas en la transmisión de información son:
...
: `A`  envía un mensaje a `B`. `X` intercepta el mensaje lo modifica y lo reenvía a `B` haciéndose pasar por `A`. Otra posibilidad es que el mensaje llegue ligeramente alterado `B` sin la intervención de `X` debido a un problema en la transmisión física de los datos (corrupción de los datos).
...
# La solución: el uso de técnicas de criptografía moderna
...
# El mecanismo de encriptación y desencriptación de los datos
...
Esto explica porqué la mayoría de implementaciones de algoritmos criptográficos emplean licencias abiertas. Si en criptografía clásica es el propio proceso de manipulación de los datos que codifica el mensaje el que debe mantenerse en secreto, un caso precursor de [seguridad por oscuridad](https://es.wikipedia.org/wiki/Seguridad_por_oscuridad); en criptografía moderna la implementación de los algoritmos es absolutamente transparente, siendo la clave de encriptación (concretamente su longitud), quien otorga mayor o menor seguridad a todo el sistema.

Por otro lado, la publicación del código de estos sistemas mediante licencias abiertas confieren una interoperabilidad vital para estos sistemas, que tienen su razón de ser en la transmisión de datos entre partes heterogéneas (diferentes aplicaciones, plataformas...). Además, y aunque pueda parecer paradójico, el modelo de licencia libre proporciona un [mecanismo óptimo para la corrección de posibles agujeros de seguridad](https://es.wikipedia.org/wiki/Ley_de_Linus).
...
El problema de *interceptación* de datos queda resuelto: en el caso de que un agente no autorizado `X` interceptara el mensaje, en un nodo intermedio de la red, solo obtendría un texto totalmente ilegible.
...
Se podría sopesar la posibilidad de encriptar el mismo mensaje que contiene la clave. Sin embargo, el problema se repetiría hasta el infinito: a dicho mensaje encriptado correspondería una nueva clave que tampoco podría ser transmitida de forma segura por la red.

Para solucionar de forma definitiva este problema se conocen dos opciones:
...
1. Permiten verificar la *integridad* de un mensaje, certificando si los datos se mantienen intactos desde su origen (es decir, no han sido corrompidos o manipulados en su transmisión por el medio físico)
...
Las anteriores propiedades hacen de las *funciones hash* una herramienta ideal para un sistema de autenticación mediante contraseña:
...
El hecho de almacenar un valor *hash* en lugar de la contraseña original (ya sea en plano o mediante algun tipo de encriptación) aporta los siguientes beneficios en el caso de que nuestra plataforma sea comprometida por el acceso de un agente externo:
...
>
* Se calcula la función *hash* del mensaje con la implementación correspondiente (*MD5*, *SHA-1*...)
* Se compara el resultado con el valor de referencia precalculado en el origen de la transmisión
...
Para poder asociar un mensaje de forma unívoca con su creador, se necesita el uso de un esquema de criptografía asimétrica: la [*firma digital*](/krypto/#tocAnchor-1-11-2).
...
* `B` genera un par de claves pública/privada, pone la clave pública a disposición de `A`, y guarda en un lugar seguro su clave privada
* La parte `A` encripta un mensaje mediante la clave pública de la parte `B` y se lo envía
...
Otro problema aún más importante del cifrado asimétrico es que no puede impedir, por si mismo, un ataque del tipo *man-in-the-middle*:
...
La conclusión es que el agente `X` podría llegar a actuar como "intermediario", *man in the middle*, de las comunicaciones entre `A` y `B`, haciéndose con todas sus comunicaciones sin que ninguna de las partes afectadas llegué a sospechar nada.
...
De esta forma el mensaje y la firma forman un todo.

Ya que el proceso de encriptación asimétrico es costoso, como se vio anteriormente, la generación de un código *hash* y la encriptación de este reduce dicho coste de forma exponencial. Enviando el código encriptado y el mensaje original por separado, dejamos al destinatario la tarea (mucho menos costosa) de comprobar que el código enviado corresponde con el mensaje recibido.
...
Además, para que estos datos puedan ser asociados unívocamente a cierta clave pública es necesario el "beneplácito" de una autoridad de certificación (*CA*)
...
* La autoridad de certificación genera entonces un certificado firmando con su propia clave privada los datos y la clave de la parte `A`

El certificado de `A` ha sido expedido, a partir de ahora ya puede empezar a utilizarlo:

>
* `A` entrega su certificado (que a partir de ahora sustituirá a su clave pública) a `B`
...
La autoridad certificadora también requiere de un certificado que la haga confiable. El sistema que determina la confiabilidad de una CA se describe en [el certificado SSL/TLS](krypto/el-certificado-ssl-tls). 
...
Se utiliza un sistema de [*código de autenticación*](/krypto/#tocAnchor-1-6-4) de mensaje (*MAC*) para asegurar la integridad de los datos. Concretamente se utilizan dos implementaciones distintas según la fase de autenticación/verificación, *HMAC* y *AEAD*.

## El certificado SSL/TLS

El protocolo *SSL*/*TLS* garantiza la encriptación de nuestros datos hasta el servidor remoto propietario del certificado en cuestión (estándar *X.509*). Para ello utiliza un modelo conocido como cadena de confianza (*chain of trust*).
...
* Al final de dicha cadena de certificados se encontrará un certificado raíz (root), autofirmado por la propia *CA*, si el navegador requiere llegar hasta este último paso deberá verificar que dicha *CA* se encuentre en el listado de *CAs*
...
> 
La generación de certificados autofirmados, es una práctica común en entornos de desarrollo o integración no publicados en internet. Para evitar una advertencia del navegador se puede optar por la instalación de un certificado siguiendo un modelo de "cadena de confianza". En tal caso somos nosotros quien otorgamos confianza al certificado al margen de una *CA*.

## La aceptación de HTTPS

La complejidad de la generación de los certificados *SSL* ha sido la responsable, históricamente, de que solamente aquellas webs que manejan datos sensibles (como e-commerce) se hayan decantado por coexiones seguras HTTPS.

Actualmente, sin embargo, servicios como [*Let's encrypt*](https://es.wikipedia.org/wiki/Let%27s_encrypt) han reducido drásticamente la complejidad en la instalación de certificados lo que podría hacer cambiar esta situación.
...
Las primeras versiones de *PGP* eran conocidas por ofrecer un esquema descentralizado de *red de confianza*, en contraste con el sistema centralizado *PKI* (estándar *X.509*) de *SSL*/*TLS*. Actualmente tanto *PGP* como *GPG* han incorporado adicionalmente el modelo *PKI*.

Correo encriptado con Gmail
: 
> Si bien servicios como Gmail incorporan soporte *TLS* (sobre IMAP, por ejemplo), esto no significa que el servicio almacene los correos de forma encriptada. Para encriptar nuestros correos deberemos utilizar un cliente de correo propio con soporte para *PGP*/*GPG*. En la actualidad existen incluso extensiones de encriptación *PGP*/*GPG* para Google Chrome. Estos plugins encriptan y desencriptan los correos de forma local, mediante código javascript, haciendo posible la encriptación accediendo directamente desde la web de Gmail.
...
La suite *GPG* (*GnuPG*) es libre, multiplataforma, y es interoperable con *PGP* en versiones recientes.
