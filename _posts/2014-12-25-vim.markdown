...
<!--
/* TODO:
ft motions, files (ctrlp + root plugin + nerdtree), falta explicar search + /e /C parametros
*/
-->
...
Custom command `GrepLastSearch` (to set in .vimrc)
: `command GrepLastSearch execute 'vimgrep /' . @/ . '/ **'`

> visual-star search recommended, which escapes special characters passing to vimgrep a `noverymagic` string

> To grep for nonexistent string in buffer, write the string in command window and search for it with visual-star plugin
