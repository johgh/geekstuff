...
> Ver más términos [aquí](https://www.atlassian.com/git/glossary/terminology/)
...
> #### Si ***no hay*** cambios en el repositorio remoto:
...
2) Dejar limpio el índice y el working tree para poder hacer pull
: 
    * **opción a**: ```git commit```
    * **opción b**: ```git stash```

3) Obtener los cambios realizados en remoto mediante ```git pull [--rebase]```
: 
    * **escenario a**: No hay conflicto. Automerge commit.
    * **escenario b**: Hay conflicto. Resolvemos con ```git mergetool``` y ejecutamos ```git commit``` una vez resuelto.

3.5) [Opcional]. Si hemos ejecutado ```git stash``` podemos recuperar ahora el stash guardado:
: ```git stash apply```

4) Comprobar de nuevo si se han producido cambios en remoto:
: ```git pull```

5) Publicar nuestros commits en remoto:
: ```git push```
...
Después de abortar un merge "innecesario" (ver arriba)
: {% highlight sh %}
    $ git pull --rebase
{% endhighlight %}
