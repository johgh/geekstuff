...
: 
    `W` go to the next WORD<br />
    `E` go to the end of the current WORD<br />
    `B` go to the previous WORD<br />
    `gE` go to the end of the previous WORD<br />
...
### Plugin text objects
`/` (vim-textobj-lastpat)
: for the region of the last pattern searched. (As of vim 7.4 you can use cgn/dgn/gUgn instead, see `:h gn`)

`j` (vim-textobj-brace)
: for the closest region between any of `()` `[]` `{}` `<>`
...
## Surround
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/tpope/vim-surround)
...
## Multiple cursors
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/terryma/vim-multiple-cursors)

enter multiple line mode
: `<C-v>` or `V` (`V` will put the cursor at the beginnig of all lines), and press `<C-n>`

> You will enter at multiple cursor `normal` mode, see changing between modes below

enter multiple "selection" mode
: Put cursor at word and press `<C-n>` / Select word in visual mode and press `<C-n>`

> You will enter at multiple cursor `visual` mode, see changing between modes below
...
### Changing between multiple cursor modes

change between visual and normal mode
: `v`
...
exit all multiple cursor modes
: `<esc>`

## Expand Region
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/terryma/vim-expand-region)

expand visual selection
: `+`

shrink visual selection
: `<C-+>` (default mapping is `_`)

## Commentary
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/tpope/vim-commentary)
...
## Exchange
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/tommcdo/vim-exchange)
...
## DelimitMate
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/Raimondi/delimitMate)
...
## Utilsnips
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/SirVer/ultisnips)
...
## PhpDocumentor (PDV)
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/tobyS/pdv)

`<leader>c`
: call PDV snippet to doc function

## Abolish
> [<i class="fas fa-external-link-alt"></i> Github project](https://github.com/tpope/vim-abolish)
...
## Useful commands
Copy current file to clipboard (Linux)
: `:%w !xsel -b`

Copy current file to clipboard (Mac)
: `:%w !pbcopy`

> Useful when no clipboard support is pre-compiled

Save with sudo from a not sudoed file
: `:w !sudo tee % >/dev/null`
...
: `:Gdiff` and put selected changes to the left pane (`index`) and save, see 2-way diff [above](/vim/#tocAnchor-1-18)

## Reviewing changes
Review changes from staged/unstaged files
: `:Gstatus` and press `D` over the files to see changes 

See commited files of a specific commit
: `:Gedit <commit_hash>`, or `:Gedit HEAD~0` to see current commited files. Press enter on a file to see changes from last commit on a 2-way diff
