...
{:.center}
![Git workflow](/workflow.png "Git workflow")
...
Para guardar passwords con Gnome keyring (sólo para https, no ssh)
: {% highlight sh %}
...
> Un ejemplo de configuración completa (alias incluídos) [aquí](/gitconfig)
...
Finalmente publicaremos nuestros commits en remoto:
: `git status` (comprobamos que todos los conflictos están marcados como resueltos)
`git push`

> Si fallará el `git push` quiere decir que se han producido nuevos cambios en remoto mientras mergeabamos, deberemos volver a hacer pull: `git pull` + [resolución de conflictos] + `git push`
