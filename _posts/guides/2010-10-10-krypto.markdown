...
El propósito de este texto es la introducción de los conceptos criptográficos sobre los que están basados la seguridad en la transmisión de información a través de una red.

El alcance de esta guía es múltiple:

1. Por un lado se establecen los problemas básicos en la transmisión de datos y que esquemas (mecanismos) criptográficos podemos utilizar para solucionarlos.

2. En segundo lugar se documenta con detalle el funcionamiento de dichos esquemas, analizando sus puntos fuertes y sus puntos débiles.

3. Finalmente, se enumeran y se desriben (a gandes rasgos) un par de implementaciones de los esquemas descritos anteriormente.

El enfoque de este documento, aunque riguroso y detallado, no entra en un nivel matemático formal, por lo que es una lectura indicada para alguien sin ningún conocimiento previo en la materia.
...
`Interceptación`
: `A` envía a `B` un mensaje. Como este mensaje se envía a través de una red no segura, este puede ser interceptado por `X` mediante la captura de los paquetes de datos de la red (`sniffing` de datos)
...
El funcionamiento de dicho esquema es el siguiente:
...
El hecho de almacenar un valor hash en lugar de la contraseña (ya sea en plano o encriptada) aporta los siguientes beneficios en el caso de que nuestro almacenamiento se vea comprometido por el acceso de un agente externo:

1. El intruso nunca podrá calcular el valor original de las contraseñas a partir de su código hash
2. Tampoco podrá extraer conclusiones, simplemente a partir de los valores hash, ya que:
...
Para verificar la integridad de un mensaje deberemos contar previamente con un valor hash de referencia asociado al mensaje recibido. El procedimiento de verificación consta de 2 simples pasos:

>
* Se calcula la función hash del mensaje con la implementación que corresponda (MD5, SHA-1...)
...
GnuPG, además, está integrado en programas como Evolution y KMail (Linux), y existe una extensión llamada Enigmail para varios programas de Mozilla (multiplataforma).
